...
class HB_GUI:
    ...
    def pack_Pitch(self, master, Location):
        ...
        self.theta_ref_slider = self.create_slider_control(pitch_frame, "Theta Ref:", self.theta_decrease, self.theta_increase, P.theta_ref_min, P.theta_ref_max, P.theta_ref_step, P.theta_ref)
        self.theta_kp_slider = self.create_slider_control(pitch_frame, "Theta Kp:", self.theta_kp_decrease, self.theta_kp_increase, P.theta_kp_min, P.theta_kp_max, P.theta_kp_step, P.theta_kp)
        self.theta_kd_slider = self.create_slider_control(pitch_frame, "Theta Kd:", self.theta_kd_decrease, self.theta_kd_increase, P.theta_kd_min, P.theta_kd_max, P.theta_kd_step, P.theta_kd)
        self.theta_ki_slider = self.create_slider_control(pitch_frame, "Theta Ki:", self.theta_ki_decrease, self.theta_ki_increase, P.theta_ki_min, P.theta_ki_max, P.theta_ki_step, P.theta_ki)
    ...
    def pack_Roll(self,master,Location):
        ...
        self.phi_ref_slider = self.create_slider_control(phiPD_frame, "Phi Ref:", self.phi_decrease, self.phi_increase, P.phi_ref_min, P.phi_ref_max, P.phi_ref_step, P.phi_ref)
        self.phi_kp_slider = self.create_slider_control(phiPD_frame, "Phi Kp:", self.phi_kp_decrease, self.phi_kp_increase, P.phi_kp_min, P.phi_kp_max, P.phi_kp_step, P.phi_kp)
        self.phi_kd_slider = self.create_slider_control(phiPD_frame, "Phi Kd:", self.phi_kd_decrease, self.phi_kd_increase, P.phi_kd_min, P.phi_kd_max, P.phi_kd_step, P.phi_kd)
    ...
    def pack_Yaw(self,master,Location):
        ...
        self.psi_ref_slider = self.create_slider_control(psiPID_frame, "Psi Ref:", self.psi_decrease, self.psi_increase, P.psi_ref_min, P.psi_ref_max, P.psi_ref_step, P.psi_ref)
        self.psi_kp_slider = self.create_slider_control(psiPID_frame, "Psi Kp:", self.psi_kp_decrease, self.psi_kp_increase, P.psi_kp_min, P.psi_kp_max, P.psi_kp_step, P.psi_kp)
        self.psi_kd_slider = self.create_slider_control(psiPID_frame, "Psi Kd:", self.psi_kd_decrease, self.psi_kd_increase, P.psi_kd_min, P.psi_kd_max, P.psi_kd_step, P.psi_kd)
        self.psi_ki_slider = self.create_slider_control(psiPID_frame, "Psi Ki:", self.psi_ki_decrease, self.psi_ki_increase, P.psi_ki_min, P.psi_ki_max, P.psi_ki_step, P.psi_ki)
    ...
    def pack_Action(self,master,Location):
        ...
        roll_button = tk.Button(action_frame, text="Roll", command=self.set_roll, width=self.w_button_2, height=self.h_button)
        roll_button.pack(side=tk.LEFT, padx=5)

        yaw_button = tk.Button(action_frame, text="Yaw", command=self.set_yaw, width=self.w_button_2, height=self.h_button)
    ...
    def pack_Control(self,master,Location):
        ...
        zero_button = tk.Button(HB_Control_frame, text="Zero", command=self.send_zero, width=self.w_button_3, height=self.h_button)
        zero_button.pack(side=tk.LEFT, padx=5)

        roll_button = tk.Button(HB_Control_frame, text="Pause", command=self.send_pause, width=self.w_button_3, height=self.h_button)
        roll_button.pack(side=tk.LEFT, padx=5)

        yaw_button = tk.Button(HB_Control_frame, text="Play", command=self.send_play, width=self.w_button_3, height=self.h_button)
        yaw_button.pack(side=tk.LEFT, padx=5)
    ...
    def pack_Action_Full(self,master,Location):
        ...
        roll_button = tk.Button(full_action_frame, text="Roll", command=self.set_roll_full, width=self.w_button_4, height=self.h_button)
        roll_button.pack(side=tk.LEFT, padx=5)

        pitch_button = tk.Button(full_action_frame, text="Pitch", command=self.set_pitch_full, width=self.w_button_4, height=self.h_button)
        pitch_button.pack(side=tk.LEFT, padx=5)

        yaw_button = tk.Button(full_action_frame, text="Yaw", command=self.set_yaw_full, width=self.w_button_4, height=self.h_button)
        yaw_button.pack(side=tk.LEFT, padx=5)

        full_button = tk.Button(full_action_frame, text="Full", command=self.set_full, width=self.w_button_4, height=self.h_button)
