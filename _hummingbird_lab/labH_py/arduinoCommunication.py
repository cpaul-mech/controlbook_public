...
def send_gains(ser):
    if P.Pause and not P.Play:
        GUI_send = chr(P.startMarker) + P.char_pause + P.char_pause + P.char_pause + chr(P.endMarker)
        send_msg(ser,GUI_send)
        
    if P.Play and not P.Pause:
        GUI_send = chr(P.startMarker) + P.char_play + P.char_play + P.char_play + chr(P.endMarker)
        send_msg(ser,GUI_send)
        P.Play = False

    if P.Zero:
        GUI_send = chr(P.startMarker) + P.char_zero + P.char_zero + P.char_zero+ chr(P.endMarker)
        send_msg(ser,GUI_send)
        P.Zero = False

    if P.controller == P.controller_km and not P.Pause:
        GUI_send = (
            chr(P.startMarker) + P.char_km + 
            P.char_val1 + str(P.km) + 
            chr(P.endMarker)
        )
        send_msg(ser,GUI_send)
    ...
    if P.controller == P.controller_roll and not P.Pause:
        GUI_send = (
            chr(P.startMarker) + P.char_roll + 
            P.char_val1 + str(P.km) + 
            P.char_val2 + str(round(P.phi_ref*np.pi/180,4)) + 
            P.char_val3 + str(P.phi_kp) + 
            P.char_val4 + str(P.phi_kd) + 
            P.char_val5 +
            chr(P.endMarker)
        )
        send_msg(ser,GUI_send)     
    ...
    if P.controller == P.controller_pitch and not P.Pause:
        GUI_send = (
            chr(P.startMarker) + P.char_pitch + 
            P.char_val1 + str(P.km) + 
            P.char_val2 + str(round(P.theta_ref*np.pi/180,4)) + 
            P.char_val3 + str(P.theta_kp) + 
            P.char_val4 + str(P.theta_ki) + 
            P.char_val5 + str(P.theta_kd) + 
            P.char_val6 +
            chr(P.endMarker)
        )
        send_msg(ser,GUI_send)  
    ...
    if P.controller == P.controller_yaw and not P.Pause:
        GUI_send = (
            chr(P.startMarker) + P.char_yaw +
            P.char_val1 + str(P.km) + 
            P.char_val2 + str(round(P.psi_ref*np.pi/180,4)) + 
            P.char_val3 + str(P.psi_kp) + 
            P.char_val4 + str(P.psi_ki) + 
            P.char_val5 + str(P.psi_kd) + 
            P.char_val6 +
            chr(P.endMarker)
        )
        send_msg(ser,GUI_send)
    ...
    if P.controller == P.controller_full and not P.Pause:
        GUI_send = (
            chr(P.startMarker) + P.char_full +
            P.char_val1 + str(round(P.theta_ref*np.pi/180,4)) + 
            P.char_val2 + str(round(P.psi_ref*np.pi/180,4)) + 
            P.char_val3 +
            chr(P.endMarker)
        )
